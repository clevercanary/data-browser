...
interface UseEntityBackPageResponse<T> {
  isLoading: boolean;
  response?: T;
}

/**
 * Hook handling the load and transformation of the values used by back pages. If the current entity loaded statically,
...
export const useFetchEntity = <T,>(
...
): UseEntityBackPageResponse<T> => {
  const { backPage, path, staticLoad } = useFetcher();
  ...
  useEffect(() => {
    if (!staticLoad && uuid) {
      run(backPage(uuid, path));
    }
  }, [backPage, path, run, staticLoad, uuid]);
