...
export const usePagination = (data?: AzulEntitiesResponse): Pagination => {
  ...
  const nextPage = useCallback(async () => {
    if (data?.pagination?.next) {
      ...
      run(fetchEntitiesFromURL(data.pagination.next, authState.token));
    }
  }, [authState.token, data, fetchEntitiesFromURL, run]);

  // Create callback for previous page action.
  const previousPage = useCallback(async () => {
    if (data?.pagination?.previous) {
      setCurrentPage((s) => s - 1);
      run(fetchEntitiesFromURL(data.pagination.previous, authState.token));
    }
  }, [authState.token, data, fetchEntitiesFromURL, run]);
