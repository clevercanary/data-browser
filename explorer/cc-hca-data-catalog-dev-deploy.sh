...
# Build AnVIL
rm -rf ./out
npm run build:hca-catalog
mv out/explore/* build/explore


export BUCKET=s3://hca-project-catalog.dev.clevercanary.com/
export SRCDIR=build/

aws s3 sync  $SRCDIR $BUCKET --delete --profile excira
aws cloudfront create-invalidation --distribution-id EPETO2NZ45EHY --paths "/*" --profile excira